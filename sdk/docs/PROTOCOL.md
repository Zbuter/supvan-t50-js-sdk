...
1. 先按 `materialWidth * dotsPerMm` 校验页面内容宽度；T50 BLE 逻辑帧始终建立宽 `384` 点、高 `materialHeight * dotsPerMm` 点的打印头工作区。40 mm 标签的 320 点内容会居中到 384 点工作区，不能把帧行宽改成 320 点。
2. 按公共 `direction` 旋转并居中，应用水平/垂直偏移；源图超过页面内容宽度时拒绝，50 mm 页面中超过 384 点打印头的部分会等比缩到 384 点。
3. 转为最终传输方向（相对源图逆时针 90 度）。
4. 每列固定打包 `48` 字节（`384 / 8`），黑点为 1。该行跨度是固件协议字段，不能随标签宽度变化。
...
USB 图像数据不走 BLE 的 LZMA/`AA BB` 包。传输块由一组完整帧组成，HID transport 再拆成 64 字节报告。输入报告的 WebHID `data` 已经不包含 HID report ID，但 T50 返回数据仍带一个协议前导字节；transport 去掉该字节后再交给上层解析。

每份 64 字节输入报告因此提供 63 字节逻辑数据。状态响应虽然只有 8 字节，但读取时必须消费完整的 63 字节，否则报告末尾的填充会残留到下一次状态查询。
